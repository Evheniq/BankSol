...
contract Bank is Erc20, Ownable {
    ...
    function addRequestCredit(uint _amountCredit) public{
        requestCredits[msg.sender] = creditClient({amountCredit: _amountCredit, thenAPY: APY, date: now});
    }
    ...
    function deposit () public payable{
        mintErc(msg.sender, msg.value); // mint 1 ETH = 1 MBT, like us bonds
        acceptedDeposits[msg.sender] = depositClient(msg.value, APY, now); // Assume that 1 person, 1 deposit
    }
    
    function returnDeposit(uint _value) public{
